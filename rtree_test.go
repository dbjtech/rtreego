...
func mustRect(p Point, widths []float32) Rect {
...
var chooseLeafNodeTests = []struct {
	...
	{
		mustRect(Point{1, 1, 1}, []float32{1, 1, 1}),
		mustRect(Point{-1, -1, -1}, []float32{0.5, 0.5, 0.5}),
		mustRect(Point{3, 4, -5}, []float32{2, 0.9, 8}),
		1,
		"clear winner",
		1,
	},
	{
		mustRect(Point{-1, -1.5, -1}, []float32{0.5, 2.5025, 0.5}),
		mustRect(Point{0.5, 1, 0.5}, []float32{0.5, 0.815, 0.5}),
		mustRect(Point{3, 4, -5}, []float32{2, 0.9, 8}),
		1,
		"leaves tie",
		1,
	},
	{
		mustRect(Point{-1, -1.5, -1}, []float32{0.5, 2.5025, 0.5}),
		mustRect(Point{0.5, 1, 0.5}, []float32{0.5, 0.815, 0.5}),
		mustRect(Point{-1, -2, -3}, []float32{2, 4, 6}),
		2,
		"leaf contains obj",
		1,
	},
...
func TestPickSeeds(t *testing.T) {
	entry1 := entry{bb: mustRect(Point{1, 1}, []float32{1, 1})}
	entry2 := entry{bb: mustRect(Point{1, -1}, []float32{2, 1})}
	entry3 := entry{bb: mustRect(Point{-1, -1}, []float32{1, 2})}
...
func TestPickNext(t *testing.T) {
	leftEntry := entry{bb: mustRect(Point{1, 1}, []float32{1, 1})}
	left := &node{entries: []entry{leftEntry}}

	rightEntry := entry{bb: mustRect(Point{-1, -1}, []float32{1, 2})}
	right := &node{entries: []entry{rightEntry}}

	entry1 := entry{bb: mustRect(Point{0, 0}, []float32{1, 1})}
	entry2 := entry{bb: mustRect(Point{-2, -2}, []float32{1, 1})}
	entry3 := entry{bb: mustRect(Point{1, 2}, []float32{1, 1})}
...
func TestSplit(t *testing.T) {
	entry1 := entry{bb: mustRect(Point{-3, -1}, []float32{2, 1})}
	entry2 := entry{bb: mustRect(Point{1, 2}, []float32{1, 1})}
	entry3 := entry{bb: mustRect(Point{-1, 0}, []float32{1, 1})}
	entry4 := entry{bb: mustRect(Point{-3, -3}, []float32{1, 1})}
	entry5 := entry{bb: mustRect(Point{1, -1}, []float32{2, 2})}
...
func TestSplitUnderflow(t *testing.T) {
	entry1 := entry{bb: mustRect(Point{0, 0}, []float32{1, 1})}
	entry2 := entry{bb: mustRect(Point{0, 1}, []float32{1, 1})}
	entry3 := entry{bb: mustRect(Point{0, 2}, []float32{1, 1})}
	entry4 := entry{bb: mustRect(Point{0, 3}, []float32{1, 1})}
	entry5 := entry{bb: mustRect(Point{-50, -50}, []float32{1, 1})}
	entries := []entry{entry1, entry2, entry3, entry4, entry5}
	n := &node{entries: entries}
...
func TestAssignGroupLeastEnlargement(t *testing.T) {
	r00 := entry{bb: mustRect(Point{0, 0}, []float32{1, 1})}
	r01 := entry{bb: mustRect(Point{0, 1}, []float32{1, 1})}
	r10 := entry{bb: mustRect(Point{1, 0}, []float32{1, 1})}
	r11 := entry{bb: mustRect(Point{1, 1}, []float32{1, 1})}
	r02 := entry{bb: mustRect(Point{0, 2}, []float32{1, 1})}
...
func TestAssignGroupSmallerArea(t *testing.T) {
	r00 := entry{bb: mustRect(Point{0, 0}, []float32{1, 1})}
	r01 := entry{bb: mustRect(Point{0, 1}, []float32{1, 1})}
	r12 := entry{bb: mustRect(Point{1, 2}, []float32{1, 1})}
	r02 := entry{bb: mustRect(Point{0, 2}, []float32{1, 1})}
...
func TestAssignGroupFewerEntries(t *testing.T) {
	r0001 := entry{bb: mustRect(Point{0, 0}, []float32{1, 2})}
	r12 := entry{bb: mustRect(Point{1, 2}, []float32{1, 1})}
	r22 := entry{bb: mustRect(Point{2, 2}, []float32{1, 1})}
	r02 := entry{bb: mustRect(Point{0, 2}, []float32{1, 1})}
...
func TestInsertRepeated(t *testing.T) {
	var things []Spatial
	for i := 0; i < 10; i++ {
		things = append(things, mustRect(Point{0, 0}, []float32{2, 1}))
	}

	for _, tc := range tests(2, 3, 5, things...) {
		t.Run(tc.name, func(t *testing.T) {
			rt := tc.build()
			rt.Insert(mustRect(Point{0, 0}, []float32{2, 1}))
		})
	}
}
...
func TestInsertNoSplit(t *testing.T) {
	rt := NewTree(2, 3, 3)
	thing := mustRect(Point{0, 0}, []float32{2, 1})
	rt.Insert(thing)
...
func TestInsertSplitRoot(t *testing.T) {
	rt := NewTree(2, 3, 3)
	things := []Rect{
		mustRect(Point{0, 0}, []float32{2, 1}),
		mustRect(Point{3, 1}, []float32{1, 2}),
		mustRect(Point{1, 2}, []float32{2, 2}),
		mustRect(Point{8, 6}, []float32{1, 1}),
		mustRect(Point{10, 3}, []float32{1, 2}),
		mustRect(Point{11, 7}, []float32{1, 1}),
	}
...
func TestInsertSplit(t *testing.T) {
	...
	things := []Rect{
		...
		mustRect(Point{10, 3}, []float32{1, 2}),
		mustRect(Point{11, 7}, []float32{1, 1}),
		mustRect(Point{10, 10}, []float32{2, 2}),
	}
	for _, thing := range things {
		rt.Insert(thing)
	}
...
func TestInsertSplitSecondLevel(t *testing.T) {
	...
	things := []Rect{
		...
		mustRect(Point{10, 3}, []float32{1, 2}),
		mustRect(Point{11, 7}, []float32{1, 1}),
		mustRect(Point{0, 6}, []float32{1, 2}),
		mustRect(Point{1, 6}, []float32{1, 2}),
		mustRect(Point{0, 8}, []float32{1, 2}),
		mustRect(Point{1, 8}, []float32{1, 2}),
	}
	for _, thing := range things {
		rt.Insert(thing)
	}
...
func TestBulkLoadingValidity(t *testing.T) {
	var things []Spatial
	for i := float32(0); i < float32(100); i++ {
		things = append(things, mustRect(Point{i, i}, []float32{1, 1}))
	}
...
func TestFindLeafDoesNotExist(t *testing.T) {
	...
	obj := mustRect(Point{99, 99}, []float32{99, 99})
...
func TestChooseNodeNonLeaf(t *testing.T) {
	...
	obj := mustRect(Point{0, 10}, []float32{1, 2})
	e := entry{obj, nil, obj}
...
func TestDeleteWithComparator(t *testing.T) {
	...
	things := []Spatial{
		&IDRect{"1", mustRect(Point{0, 0}, []float32{2, 1})},
		&IDRect{"2", mustRect(Point{3, 1}, []float32{1, 2})},
		&IDRect{"3", mustRect(Point{1, 2}, []float32{2, 2})},
		&IDRect{"4", mustRect(Point{8, 6}, []float32{1, 1})},
		&IDRect{"5", mustRect(Point{10, 3}, []float32{1, 2})},
		&IDRect{"6", mustRect(Point{11, 7}, []float32{1, 1})},
		&IDRect{"7", mustRect(Point{0, 6}, []float32{1, 2})},
		&IDRect{"8", mustRect(Point{1, 6}, []float32{1, 2})},
		&IDRect{"9", mustRect(Point{0, 8}, []float32{1, 2})},
		&IDRect{"10", mustRect(Point{1, 8}, []float32{1, 2})},
	}
...
func TestDeleteThenInsert(t *testing.T) {
	tol := float32(1e-3)
	rects := []Rect{
		mustRect(Point{3, 1}, []float32{tol, tol}),
		mustRect(Point{1, 2}, []float32{tol, tol}),
		mustRect(Point{2, 6}, []float32{tol, tol}),
		mustRect(Point{3, 6}, []float32{tol, tol}),
		mustRect(Point{2, 8}, []float32{tol, tol}),
	}
	things := []Spatial{}
	for i := range rects {
		things = append(things, &rects[i])
...
func TestSearchIntersect(t *testing.T) {
	rects := []Rect{
		...
		mustRect(Point{10, 3}, []float32{1, 2}),
		mustRect(Point{11, 7}, []float32{1, 1}),
		mustRect(Point{2, 6}, []float32{1, 2}),
		mustRect(Point{3, 6}, []float32{1, 2}),
		mustRect(Point{2, 8}, []float32{1, 2}),
		mustRect(Point{3, 8}, []float32{1, 2}),
	}
	things := []Spatial{}
	for i := range rects {
		things = append(things, &rects[i])
...
func TestSearchIntersectWithLimit(t *testing.T) {
	...
	for _, tc := range tests(2, 3, 3, things...) {
		t.Run(tc.name, func(t *testing.T) {
			...
			bb := mustRect(Point{2, 1.5}, []float32{10, 5.5})
...
func TestSortEntries(t *testing.T) {
	objs := []Rect{
		mustRect(Point{1, 1}, []float32{1, 1}),
		mustRect(Point{2, 2}, []float32{1, 1}),
		mustRect(Point{3, 3}, []float32{1, 1})}
	entries := []entry{
		{objs[2], nil, &objs[2]},
		{objs[1], nil, &objs[1]},
		{objs[0], nil, &objs[0]},
...
func TestNearestNeighbor(t *testing.T) {
	rects := []Rect{
		mustRect(Point{1, 1}, []float32{1, 1}),
		mustRect(Point{1, 3}, []float32{1, 1}),
		mustRect(Point{3, 2}, []float32{1, 1}),
		mustRect(Point{-7, -7}, []float32{1, 1}),
		mustRect(Point{7, 7}, []float32{1, 1}),
		mustRect(Point{10, 2}, []float32{1, 1}),
	}
	things := []Spatial{}
	for i := range rects {
		things = append(things, rects[i])
...
func TestGetAllBoundingBoxes(t *testing.T) {
	...
	things := []Rect{
		...
		mustRect(Point{11, 7}, []float32{1, 1}),
		mustRect(Point{10, 10}, []float32{2, 2}),
		mustRect(Point{2, 3}, []float32{0.5, 1}),
		mustRect(Point{3, 5}, []float32{1.5, 2}),
		mustRect(Point{7, 14}, []float32{2.5, 2}),
		mustRect(Point{15, 6}, []float32{1, 1}),
		mustRect(Point{4, 3}, []float32{1, 2}),
		mustRect(Point{1, 7}, []float32{1, 1}),
		mustRect(Point{10, 5}, []float32{2, 2}),
	}
	for _, thing := range things {
		rt1.Insert(thing)
	}
